...
class Wallet extends EventEmitter {
  ...
  async _fund(mtx, options) {
    ...
    const change = await this.changeAddress(acct, options.changeAddressIndex);
  ...
  async changeAddress(acct = 0, changeAddressIndex = null) {
    const account = await this.getAccount(acct);
    if (!account)
      throw new Error('Account not found.');
    ...
    if (changeAddressIndex !== null) {
      ...
      if (changeAddressIndex > (account.changeDepth - 1)) {
        throw new Error(
          `Change address index greater than change depth (${changeAddressIndex} > ${account.changeDepth - 1}).`
        );
      }

      const changeKey = account.deriveChange(changeAddressIndex);

      if (!changeKey) {
        throw new Error(`Change address with index ${changeAddressIndex} not found.`);
      }

      return changeKey.getAddress();
    }
